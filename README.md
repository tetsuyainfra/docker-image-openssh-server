...
[![Docker Image Version](https://img.shields.io/docker/v/tetsuyainfra/openssh-server?label=amd64&arch=amd64)](https://hub.docker.com/r/tetsuyainfra/openssh-server/tags)
[![Docker Image Version](https://img.shields.io/docker/v/tetsuyainfra/openssh-server?label=arm64&arch=arm64)](https://hub.docker.com/r/tetsuyainfra/openssh-server/tags)
...
- [x] RUN  --mount=type=cache,target=/var/cache/apt,sharing=locked してbuildキャッシュさせる
  ...
  - [x] キャッシュ強制 echo 'Binary::apt::APT::Keep-Downloaded-Packages "true";' > /etc/apt/apt.conf.d/keep-cache
- [x] github actionsに対応させる
  - [x] branch=main以外はbuildだけにする
  - [x] 不要なTAGを削除するワークフローを作る -> やらなくてよくないか説でてきた
  - [x] 不要なcacheを削除するワークフローを作る
